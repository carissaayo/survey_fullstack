...
interface PaginationLinksProps {
  meta: Meta;
  onPageClick: (link: { url: string }) => void;
}

const PaginationLinks = ({ meta, onPageClick }: PaginationLinksProps) => {
              ...
              {meta.links &&
                meta.links.map((link, ind) => (
                  <a
                    ...
                    key={ind}
                    aria-current="page"
                    className={` relative z-10 flex items-center border   px-4 py-2 text-sm font-medium focus:z-20 hover:bg-gray-50  ${
                      ind === 0 ? "rounded-l-md " : ""
                    } 
                      ${ind === meta.links.length - 1 ? "rounded-r-md " : ""} 
                      ${
                        link.active
                          ? "border-indigo-500 bg-indigo-50 text-indigo-600  z-30"
                          : ""
                      }`}
